...
// biome-ignore lint/suspicious/noUnsafeDeclarationMerging: the class intentionally implements the same-named interface to expose its shape as the public API type
...
export class Client {
    ...
    async call(
        ...
        if (operation.tags?.includes("stream")) {
    ...
    private _httpRequest(
        ...
        const headers: { [key: string]: string } = option.headers || {};
        ...
        if (this.userAgent === "") {
            ...
            headers["User-Agent"] = `${this.userAgent} ${this._userAgent}`;
        }

        if (headers["X-Mirakurun-Priority"] === undefined) {
            if (option.priority === undefined) {
                option.priority = this.priority;
            }
            headers["X-Mirakurun-Priority"] = option.priority.toString(10);
        }
