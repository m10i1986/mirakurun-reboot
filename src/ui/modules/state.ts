...
import type { JoinParams, NotifyParams } from "../../../lib/Mirakurun/rpc.d";
...
class State extends EventEmitter<StateEventTypes> {
    ...
    async fetchStatus(): Promise<Status> {
        const status: Status = await this._rpc.call("getStatus");
        this.status = status;
        this.emit("status", status);

        // version
        if (this.version !== ".." && this.version !== status.version) {
            location.reload();
            return status;
        }
    ...
    async fetchReserves(): Promise<Reserve[]> {
        this.reserves.splice(0, this.reserves.length, ...(await this._rpc.call("getReserves")));
        this.emit("reserves", this.reserves);
        return this.reserves;
    }

    async fetchRecorded(): Promise<Recorded[]> {
        this.recorded.splice(0, this.recorded.length, ...(await this._rpc.call("getRecorded")));
        this.emit("recorded", this.recorded);
        return this.recorded;
    }

    async fetchEncodes(): Promise<EncodeItem[]> {
        this.encodes.splice(0, this.encodes.length, ...(await this._rpc.call("getEncodes")));
        this.emit("encode", this.encodes);
        return this.encodes;
    }
    ...
    async fetchServerConfig(): Promise<ConfigServer> {
        const serverConfig: ConfigServer = await (await fetch("/api/config/server")).json();
    ...
    private _initRPC() {
        this._rpc = new RPCClient(`${location.protocol === "https:" ? "wss:" : "ws:"}//${location.host}/rpc`, {
            protocols: undefined,
        ...
        rpc.on("connected", async () => {
            ...
            await this.fetchRecorded();
            await this.fetchEncodes();
        });
        ...
        rpc.methods.set("events", async (_socket, { array }: NotifyParams<Event>) => {
            ...
            let recordedUpdated = false;
            let encodesUpdated = false;
            ...
            for (const event of array) {
                switch (event.resource) {
                    ...
                    case "reserve": {
                        const reserve = event.data as Reserve;
                        ...
                        if (event.type === "remove") {
                            ...
                            this.reserves.splice(index, 1, reserve);
                        }
                        reservesUpdated = true;
                        break;
                    }
                    case "recorded": {
                        const recorded = event.data as Recorded;
                        const index = this.recorded.findIndex((r) => r.id === recorded.id);
                        if (event.type === "remove") {
                            if (index !== -1) {
                            ...
                            this.recorded.splice(index, 1, recorded);
                        }
                        recordedUpdated = true;
                        break;
                    }
                    case "encode": {
                        const encode = event.data as EncodeItem;
                        const index = this.encodes.findIndex((e) => e.id === encode.id);
                        if (event.type === "remove") {
                            if (index !== -1) {
                                this.encodes.splice(index, 1);
                            }
                        } else if (index === -1) {
                            this.encodes.push(encode);
                        } else {
                            this.encodes.splice(index, 1, encode);
                        }
                        encodesUpdated = true;
                        break;
                    }
        ...
        // ログイベントの処理
        rpc.methods.set("logs", async (_socket, { array }: NotifyParams<string>) => {
    ...
    private _updateIdleStatus() {
        ...
        const isActive = this.tuners.some(
            (tuner) => tuner.isUsing === true && tuner.users.some((user) => user.priority !== -1),
        );
        if (isActive) {
            statusName = "Active";
