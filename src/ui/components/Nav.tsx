...
export const Nav: React.FC<NavProps> = ({ pathLv1 }) => {
    ...
    const [recordingCount, setRecordingCount] = useState<number>(state.reserves.filter((reserve) => reserve.state === "recording").length);
    useEffect(() => {
        const onReserves = () => {
            setRecordingCount(state.reserves.filter((reserve) => reserve.state === "recording").length);
        };
    ...
    return (
        <Navbar className="component-nav">
            ...
            <Navbar.Group align={Alignment.END}>
                ...
                <Button variant="minimal" {...getNavbarButtonProps("recordings")} icon="mobile-video" title="録画" text={
                    <>
                        録画
                        {recordingCount !== 0 && <span className="badge">{recordingCount}</span>}
                    </>
                } />
