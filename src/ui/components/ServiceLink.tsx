...
export const ServiceLink: React.FC<ServiceLinkProps> = ({ globalId, date, time, ...props }) => {
    ...
    return (
        <div className={className} {...props}>
            ...
            <Link className={service ? undefined : "bp5-skeleton"} title="EPG 番組表 (週間)" to={to}>
                {service ? `${service.name.normalize("NFKC")} (${service.channel ? channelTypeMap[service.channel.type] : ""})` : "サービス名..."}
            </Link>
        </div>
    );
};
