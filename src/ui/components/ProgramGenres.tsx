...
export const ProgramGenres: React.FC<ProgramGenresProps> = ({ genres }) => {
    ...
    for (const genre of genres) {
        const lv1Text = Genre1Map[genre.lv1 as keyof typeof Genre1Map];
        if (!lv1Text) {
            continue;
        }

        const un2Text = GenreUN2Map[((genre.lv1 * 0x1000) + (genre.lv2 * 0x100) + (genre.un1 * 0x10) + genre.un2) as keyof typeof GenreUN2Map];
        if (un2Text) {
        ...
        const lv2Text = Genre2Map[((genre.lv1 * 0x10) + genre.lv2) as keyof typeof Genre2Map];
