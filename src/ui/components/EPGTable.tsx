...
export const EPGTable: React.FC<EPGTableProps> = ({ date, channelType, globalServiceId, defaultProgramId, defaultTime }) => {
    ...
    // null = loading, [] = empty
    const [programId, setProgramId] = useState<number | null>(defaultProgramId || null);
    const [time, setTime] = useState<number | null>(defaultTime || null);
    const [services, setServices] = useState<Service[] | null>(null);
    const [serviceItems, setServiceItems] = useState<JSX.Element[] | null>(null);
    const [timetableCols, setTimetableCols] = useState<JSX.Element[] | null>(null);
    ...
    useEffect(() => {
        ...
        if (!timescale || !headerItem || !timescaleItem) {
            return;
        }
    ...
    useEffect(() => {
        ...
        if (!header || !timescale || !timeline || !clock || !timetable || !jumpToTimeline) {
            return;
        }

        const getPosition = () => Math.floor((Date.now() - (state.todayTime ?? 0)) / 1000 / 60 * dimensions.scaleFactor);

        // スクロール連動
        const onScroll = () => {
            header.scrollLeft = timetable.scrollLeft;
            if (channelType) {
                scrollState.left[channelType] = timetable.scrollLeft;
            }
        ...
        // スクロール初期位置セット
        const savedLeft = channelType ? scrollState.left[channelType] : -1;
    ...
    useEffect(() => {
        ...
        const startAtQuery: { $gte: number; $lt: number } = {
        ...
        if (channelType) {
            ...
            if (firstService) {
                query.serviceId = firstService.serviceId;
            }
            startAtQuery.$lt = startTime + 60 * 60 * 24 * 8 * 1000;
        }
            ...
            for (let i = 0; i < servicePrograms.length; i++) {
                ...
                if (program.genres && program.genres[0]) {
                    ...
                    for (const genre of program.genres) {
                        const un2Key = (genre.lv1 * 0x1000) + (genre.lv2 * 0x100) + (genre.un1 * 0x10) + genre.un2;
                        const un2Text: string | undefined = GenreUN2Map[un2Key as keyof typeof GenreUN2Map];
                        if (un2Text) {
                            cautions.push(genre);
                        }
                    }
        ...
        <div className="component-epg-table">
            ...
            <Button variant="outlined" className="jump-to-timeline hide" ref={jumpToTimelineRef} icon="selection"
                ...
                onClick={() => {
                    ...
                    const timetable = timetableRef.current;
                    const timeline = timelineRef.current;
                    if (!jumpToTimeline || !timetable || !timeline) {
                        return;
                    }
                    jumpToTimeline.classList.add("hide");

                    const { clientHeight } = timetable;
                    const { offsetTop } = timeline;
                    timetable.scrollTop = offsetTop - clientHeight / 4;
                }}
            />
