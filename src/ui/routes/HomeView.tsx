...
const summarizeStreamInfo = (streamInfo: StreamInfo | undefined): string => {
    if (!streamInfo) {
        return "-";
    }
...
const isEmptyStreamInfo = (streamInfo: StreamInfo | undefined): boolean => {
    if (!streamInfo) {
        return true;
    }
...
const StatusSection: React.FC<{ status: Status | undefined }> = ({ status }) => {
    if (!status) {
...
const TunersSection: React.FC<{
    ...
    const [streamDetail, setStreamDetail] = useState<{ userId: string; info: StreamInfo } | null>(null);
...
export const HomeView: React.FC = () => {
    ...
    const [status, setStatus] = useState<Status | undefined>(state.status);
    ...
    useEffect(() => {
        const onStatus = () => {
            setStatus(state.status ? { ...state.status } : undefined);
        };
