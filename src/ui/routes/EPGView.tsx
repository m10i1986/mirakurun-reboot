...
export const EPGView: React.FC = () => {
    ...
    const { navigate, searchParams } = state as { navigate: NonNullable<typeof state.navigate>; searchParams: NonNullable<typeof state.searchParams> };

    const [channelType, setChannelType] = useLocalStorageState<ChannelType | null>("EPG.channelType", "GR");
    const [programId, setProgramId] = useState<number | null>(null);
    const [time, setTime] = useState<number | null>(null);
    const globalServiceId = (params.globalServiceId ? parseInt(params.globalServiceId, 10) : NaN) || null;
    const programIdQuery = searchParams.get("programId");
    const typeQuery = searchParams.get("type");
    const dateQuery = searchParams.get("date");
    const timeQuery = searchParams.get("time");
    const now = DateTime.now();
    const isoDate = (dateQuery && /^\d{4}-\d{2}-\d{2}$/.test(dateQuery)) ? dateQuery : now.toISODate();
    ...
    return (
        <div className="route" id="route-epg">
            ...
            <div className="content no-margin">
                {globalServiceId
                    ? <EPGTable date={date} defaultTime={time ?? undefined} globalServiceId={globalServiceId} />
                    : <EPGTable date={date} defaultTime={time ?? undefined} defaultProgramId={programId ?? undefined} channelType={channelType ?? undefined} />
                }
            </div>
        </div>
    );
