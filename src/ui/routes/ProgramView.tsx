...
export const ProgramView: React.FC = () => {
    ...
    const { navigate } = state as { navigate: NonNullable<typeof state.navigate> };
    ...
    const isOnAir = program && startTime !== undefined && endTime !== null && startTime <= now && endTime >= now;
    const date = program ? DateTime.fromMillis(program.startAt).set({  hour: 0, minute: 0, second: 0, millisecond: 0 }) : null;
    const time = (program && date) ? DateTime.fromMillis(program.startAt).diff(date).toMillis() : null;
    const timeForServiceLink = (time !== null && time > (1000 * 60 * 60 * 24 - 1000 * 60 * 5)) ? 1 : time;
            ...
            <Navbar className="toolbar">
                ...
                <Navbar.Group align={Alignment.END}>
                    ...
                    {program && <>
                        ...
                        {endTime !== null && endTime > now && <ReserveButton variant="outlined" programId={program.id} />}
                    </>}
                ...
                {program && <>
                    <div className="flex">
                        <ServiceLink
                            ...
                            time={timeForServiceLink ?? undefined}
                        />
