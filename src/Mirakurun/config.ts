...
// これらは src/server.ts の setEnv で必ず設定される
const SERVER_CONFIG_PATH = process.env.SERVER_CONFIG_PATH!;
const TUNERS_CONFIG_PATH = process.env.TUNERS_CONFIG_PATH!;
const CHANNELS_CONFIG_PATH = process.env.CHANNELS_CONFIG_PATH!;
const GUIDE_CONFIG_PATH = process.env.GUIDE_CONFIG_PATH!;
...
type Guide = Readonly<apid.ConfigGuide>;
...
export async function loadServer(): Promise<Server> {
    ...
    if (!config.allowOrigins) {
        config.allowOrigins = ["https://mirakurun-secure-contexts-api.pages.dev"];
    }
...
export async function loadTuners(): Promise<Tuner[]> {
    ...
    if (existsSync(path) === false) {
        ...
        try {
            ...
            if (/which dvb-fe-tool/.test(err.message)) {
                log.warn("`dvb-fe-tool` is required to detect DVB devices. (%s)", err.message);
            } else {
                console.error(e);
            }
        }
    ...
    const tuners = await load("tuners", path);

    // YAML に `decoder:` のように値を空で書いた項目は null として読み込まれる。
    // null のままだと GET レスポンスに含まれ、UI からの保存 (PUT) 時に
    // OpenAPI のリクエスト検証 (例: decoder は type: string) で弾かれてしまう。
    // 検証はハンドラ実行前に行われ saveTuners まで到達しないため、
    // null プロパティを削除して「省略」に正規化し、往復を検証に通す。
    for (const tuner of tuners as Writable<Tuner>[]) {
        for (const [key, value] of Object.entries(tuner)) {
            if (value === null) {
                delete (tuner as Record<string, unknown>)[key];
            }
        }
    }
...
export async function loadGuide(): Promise<Guide> {
    const path = GUIDE_CONFIG_PATH;

    // ファイルが無い場合は既定値 (非表示サービス無し) を返す。
    // server.yml 等と異なり再起動不要な表示設定なので、保存されるまでは生成しない。
    if (existsSync(path) === false) {
        return { hiddenServiceIds: [] };
    }

    const config: Writable<Guide> = await load("guide", path);

    if (!Array.isArray(config.hiddenServiceIds)) {
        config.hiddenServiceIds = [];
    } else {
        config.hiddenServiceIds = config.hiddenServiceIds.filter((id) => typeof id === "number");
    }

    return config as Guide;
}

export async function saveGuide(data: Guide): Promise<void> {
    const path = GUIDE_CONFIG_PATH;
...
async function load(name: "guide", path: string): Promise<Guide>;
async function load(name: "server" | "tuners" | "channels" | "guide", path: string) {
...
async function save(name: "guide", path: string, data: Guide): Promise<void>;
async function save(name: "server" | "tuners" | "channels" | "guide", path: string, data: object): Promise<void> {
