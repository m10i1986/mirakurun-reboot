...
import { TsChar, TsCrc32, TsLogo, TsStreamLite, tsDataModule } from "@chinachu/aribts";
import _ from "./_";
...
export default class TSFilter extends EventEmitter {
    ...
    private _onOutputClose?: () => void;

    // options
    private _provideServiceId: number | null;
    private _provideEventId: number | null;
    private _parseNIT = false;
    private _parseSDT = false;
    private _parseEIT = false;
    private _targetNetworkId: number | null;
    ...
    private _provideEventTimeout?: NodeJS.Timeout;
    ...
    constructor(options: TSFilterOptions) {
        ...
        if (this._provideEventId !== null) {
            ...
            const program = _.program.get(
                getProgramItemId(this._targetNetworkId!, this._provideServiceId!, this._provideEventId),
            );
            if (program) {
        ...
        if (options.output) {
            ...
            this._output = output;
            this._onOutputClose = this._close.bind(this);
            output.once("finish", this._onOutputClose);
            output.once("close", this._onOutputClose);
        } else {
    ...
    private _onPAT(_pid: number, data: any): void {
    ...
    private _onSDT(pid: number, data: any): void {
        ...
        const _services: { networkId: number; serviceId: number; name: string; type: number; logoId: number }[] = [];
        ...
        for (const service of data.services) {
            ...
            if (_services.some((_service) => _service.serviceId === service.service_id) === false) {
                _services.push({
    ...
    private _onEIT(_pid: number, data: any): void {
    ...
    private _onTOT(_pid: number, data: any): void {
        this._streamTime = getTimeFromMJD(data.JST_time);
    }

    private _onCDT(_pid: number, data: any): void {
    ...
    private _onDSMCC(_pid: number, data: any): void {
    ...
    private _updateEpgState(data: any): void {
        ...
        // biome-ignore lint/suspicious/noAssignInExpressions: lazily initialize and reference the per-network EPG state in one expression
    ...
    private _close(): void {
        ...
        // clear buffer
        setImmediate(() => {
            delete (this as any)._packet;
            delete (this as any)._buffer;
            delete (this as any)._patsec;
            delete (this as any)._patCRC;
        });

        // clear parser instance
        this._parser.removeAllListeners();
        this._parser.end();
        delete (this as any)._parser;
        ...
        if (this._output) {
            ...
            // 外部 (Recorder 等) が登録した close リスナーを保持するため、
            // TSFilter 自身が登録したリスナーのみを除去する
            if (this._onOutputClose) {
                this._output.removeListener("finish", this._onOutputClose);
                this._output.removeListener("close", this._onOutputClose);
                delete this._onOutputClose;
            }
            delete this._output;
        }

        // clear streamInfo
        delete (this as any).streamInfo;
