...
// src/server.ts の setEnv で必ず設定される
const LOGO_DATA_DIR_PATH = process.env.LOGO_DATA_DIR_PATH!;
...
export class Service {
    ...
    static async saveLogoData(networkId: number, logoId: number, data: Uint8Array, retrying = false): Promise<void> {
        ...
        try {
            ...
            if (retrying === false) {
                ...
                return Service.saveLogoData(networkId, logoId, data, true);
            }
            throw e;
        }
    ...
    async load(): Promise<void> {
        ...
        for (const service of services) {
            if (!service.channel || service.networkId === undefined || service.serviceId === undefined) {
                updated = true;
                return;
            }
    ...
    private async _scan(channel: ChannelItem, add: boolean): Promise<void> {
        ...
        services.forEach((service) => {
            ...
            if (item !== null) {
                ...
                if (service.logoId !== undefined && service.logoId > -1) {
