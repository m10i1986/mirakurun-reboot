...
export function generateScanConfig(option: ChannelScanOption): ScanConfig | undefined {
    ...
    Object.keys(option).forEach((key) => {
        if (option[key as keyof ChannelScanOption] === undefined) {
            delete option[key as keyof ChannelScanOption];
        }
    });
    ...
    if (option.type === "GR") {
        ...
        return {
            channels: range(grOptions.startCh, grOptions.endCh).map((ch) => formatChannelName(channelFormat, ch)),
    ...
    if (option.type === "BS") {
        ...
        return {
            channels: range(bsOptions.startCh, bsOptions.endCh).map((ch) => formatChannelName(channelFormat, ch)),
    ...
    if (option.type === "CS") {
        ...
        return {
            channels: range(csOptions.startCh, csOptions.endCh).map((ch) => formatChannelName(channelFormat, ch)),
...
async function runChannelScan(
    ...
    try {
        ...
        updateStepStatus({ status: "started" as const, type }, `channel scanning... (type: "${type}")\n\n`);
        ...
        for (let i = 0; i < totalChannels; i++) {
            ...
            if (isCancellationRequested) {
                updatePhaseStatus({ status: "cancelled" as const }, "Channel scan was cancelled by user request.\n");
                break;
            }
            ...
            if (!Number.isNaN(channelNumber) && skipCh.includes(channelNumber)) {
                updateStepStatus(
                    {
            ...
            if (!refresh) {
                const existingChannels = oldChannelItems.filter(
                    (item) => item.type === type && item.channel === channel && !item.isDisabled,
                );
            ...
            try {
                ...
                services = await _.tuner.getServices(
                    <any>{
                        type,
                        channel,
                    },
        ...
        updateResultStatus({ status: "summary_new" as const, newCount }, `-> new ${newCount} channels found.\n`);
        ...
        if (!isCancellationRequested) {
            ...
            if (!dryRun) {
                ...
                updateResultStatus({ status: "restart_required" as const }, "**RESTART REQUIRED** to apply changes.\n");
            } else {
                updatePhaseStatus(
...
export const put: Operation = async (req, res) => {
    ...
    if (asyncMode) {
        ...
        runChannelScan(scanConfig, dryRun, type, refresh, undefined, skipCh).catch((error) => {
...
- BS "non" subchannel style scans and GR scans are basically the same. Note that if you scan the wrong channel range, the GR channel will be registered as BS and the BS channel will be registered as GR. This problem does not occur because CS scan uses a character string with \`CS\` added as a channel number prefix.`,
    ...
    parameters: [
        ...
        },
        {
            ...
            type: "integer",
            description:
                "Specifies the minimum number of subchannel numbers to scan. This parameter is only used if the type is `BS` and the useSubCh is `true`.",
        },
        {
            in: "query",
            name: "maxSubCh",
            type: "integer",
            description:
                "Specifies the maximum number of subchannel numbers to scan. This parameter is only used if the type is `BS` and the useSubCh is `true`.",
        },
        {
            in: "query",
            name: "useSubCh",
            ...
            default: true,
            description:
                "Specify true to use the subchannel style for channel numbers. Only used for BS scans. (e.g. BS01_0)",
        },
        {
            in: "query",
            ...
            allowEmptyValue: true,
            description:
                "Override format to use for channel name. Supports placeholders like {ch}, {ch00}, {subch}. (e.g. {ch} -> 1, BS{ch00}_{subch} -> BS01_2)",
        },
        {
            in: "query",
            name: "scanMode",
            ...
            description:
                "Channel scan mode. Use `Service` mode to create separate entries per service.\n\n" +
        ...
        },
        {
            ...
            allowEmptyValue: true,
            description:
                "If `true`, newly discovered channels will be added in disabled state.\n\n" +
        ...
        },
        {
            ...
            default: false,
            description:
                "If `true`, update the existing channel configurations without preserving them.\n" +
        ...
        },
        {
            ...
            default: false,
            description:
                "If `true`, the API returns 202 Accepted immediately and performs scan asynchronously.\n" +
