...
export class Channel {
    ...
    get(type: apid.ChannelType, channel: string): ChannelItem | null {
        ...
        channels.forEach((channel, i) => {
            ...
            for (const key in channel.commandVars) {
                if (typeof channel.commandVars[key] !== "number" && typeof channel.commandVars[key] !== "string") {
                    log.error(
                        "invalid type of property `commandVars.%s` in channel#%d (%s) configuration",
                        key,
                        i,
                        channel.name,
                    );
                ...
                readyFn: async () => {
                    ...
                    if (service.epgReady === true) {
                        ...
                        if (startup && now - service.epgUpdatedAt < 1000 * 60 * 10) {
                            // 10 mins
                            log.info(
                                "Network#%d EPG gathering has skipped because EPG is already up to date (in 10 mins)",
                                networkId,
                            );
                            return false;
                        }
                        if (now - service.epgUpdatedAt > 1000 * 60 * 60 * 12) {
                            // 12 hours
                            log.info(
                                "Network#%d EPG gathering is resuming forcibly because reached maximum pause time (12 hours)",
                                networkId,
                            );
