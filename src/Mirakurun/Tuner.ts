...
import TunerDevice, { type TunerDeviceStatus } from "./TunerDevice";
...
export class Tuner {
    ...
    async getServices(channel: ChannelItem, user: Partial<common.User> = {}): Promise<apid.Service[]> {
        ...
        return new Promise<apid.Service[]>((resolve, reject) => {
            ...
            let services: apid.Service[] | null = null;
     ...
     * リモートデータ利用 (EPG)
     */
    private async _useRemoteData(user: common.User, devices: TunerDevice[]): Promise<boolean> {
        const setting = user.streamSetting;
        if (!setting) {
            return false;
        }
     ...
     * チューナーデバイス探索
     */
    private _pickTunerDevice(devices: TunerDevice[], channel: ChannelItem, priority: number): TunerDevice | null {
        // 1. join to existing
