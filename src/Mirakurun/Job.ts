...
export class Job {
    ...
    private _checkQueue(): void {
        log.debug(
            `Job#_checkQueue() queue=${this._queuedJobItems.length} standby=${this._standbyJobItems.length} running=${this._runningJobItemSet.size}`,
        );
    ...
    private _finishJob(job: RunningJobItem, ok: boolean, error?: Error): void {
        ...
        if (this._finishedJobItems.length > this.maxHistory) {
            const removedJobs = this._finishedJobItems.splice(
                this.maxHistory,
    ...
    private async _retryJob(job: RunningJobItem): Promise<void> {
        ...
        if ((shouldRetryOnAbort || shouldRetryOnFail) && canRetry) {
            ...
            log.warn(
                `Job#_handleJobError() "${job.key}" will retry (${nextRetryCount}/${retryMax}) after ${retryDelay}ms`,
            );
...
function jobToJSON(
    status: apid.JobItem["status"],
    job: Partial<QueuedJobItem & RunningJobItem & FinishedJobItem>,
): apid.JobItem {
    const {
        ac, // omit
        fn, // omit
    ...
    return {
        ...
        isAborting: ac?.signal.aborted ?? false,
        createdAt: createdAt!,
        updatedAt: (job.finishedAt || job.startedAt || job.createdAt)!,
        duration: job.finishedAt && job.startedAt ? job.finishedAt - job.startedAt : 0,
        error: error?.message,
        ...safeJob,
    };
}
