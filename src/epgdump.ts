...
const transformStream = new stream.Transform({
    transform: function (chunk: Buffer, _encoding: string, done: (error?: Error | null) => void) {
    ...
    flush: (done) => {
        console.log("\u001b[2A");
        console.log(
            `reading - ${bytesRead} of ${size} [${Math.floor((bytesRead / size) * 100)}%] (events=${events}) [done]`,
        );
