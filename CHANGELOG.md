...
- **Recording**: Introduced a recording subsystem under `src/Recording` (`Reserve`, `Recorder`, `Recorded`) with its own config, database, and ffmpeg/util helpers, and wired it into server startup.
  - `src/Recording` 配下に録画サブシステム (`Reserve` / `Recorder` / `Recorded`) を導入し、専用の config・データベース・ffmpeg/util ヘルパーを備え、サーバー起動処理へ統合しました。

### Encode

- **Encode**: Added an encode queue subsystem (`src/Recording/Encode`) that manages encode jobs, auto-removes finished jobs, and blocks re-encoding of outputs whose source has been removed.
  - エンコードキューサブシステム (`src/Recording/Encode`) を追加し、エンコードジョブの管理、完了ジョブの自動削除、ソースが削除済みの出力に対する再エンコードのブロックを行うようにしました。

### API

- **API**: Added reservation endpoints (`/reserves`, `/reserves/{id}`, `/reserves/{id}/cancel`).
  - 予約用エンドポイント (`/reserves`, `/reserves/{id}`, `/reserves/{id}/cancel`) を追加しました。
- **API**: Added recorded endpoints for listing, streaming, downloading, and encoding (`/recorded`, `/recorded/{id}`, `/recorded/{id}/stream`, `/recorded/{id}/file`, `/recorded/{id}/encode`) and encode job endpoints (`/encode`, `/encode/{id}`), with the matching `api.yml` definitions and RPC entries.
  - 録画済みの一覧・ストリーム再生・ダウンロード・エンコード用エンドポイント (`/recorded`, `/recorded/{id}`, `/recorded/{id}/stream`, `/recorded/{id}/file`, `/recorded/{id}/encode`) とエンコードジョブ用エンドポイント (`/encode`, `/encode/{id}`) を追加し、対応する `api.yml` 定義と RPC エントリを整備しました。

### UI

- **WebUI**: Added a Recording view with playback, encode, and download controls, along with `ReserveButton` and `RecordedWatchButton` components and a navigation entry.
  - 録画ビューを追加し、再生・エンコード・ダウンロードの操作と、`ReserveButton` / `RecordedWatchButton` コンポーネントおよびナビゲーション項目を備えました。

## 5.1.1 (2026-06-19)

### UI

- **WebUI**: Rebuilt the Web UI as a route-based SPA, splitting it into Home / EPG / Program / Search / Jobs / Logs / About views and dedicated Channels / Tuners / Server config views, with the shared logic extracted into `src/ui/modules` (`state`, `common`, `constants`, etc.).
  - Web UI をルートベースの SPA として再構築し、Home / EPG / Program / Search / Jobs / Logs / About の各ビューと、Channels / Tuners / Server の設定ビューに分割しました。共通ロジックは `src/ui/modules` (`state`, `common`, `constants` など) へ切り出しました。
- **WebUI**: Added a Restart control and a VersionStatus indicator that checks the latest version on the About view.
  - 再起動操作と、About ビューで最新バージョンを確認する VersionStatus 表示を追加しました。

### Build / Toolchain

- **TypeScript**: Upgraded to TypeScript 7.0 RC and migrated the UI bundling from `ts-loader` to `esbuild-loader`.
  - TypeScript 7.0 RC へ更新し、UI のバンドルを `ts-loader` から `esbuild-loader` へ移行しました。
- **Lint**: Applied Biome lint fixes (`node:` protocol imports, template literals) and removed the `esModuleInterop` config along with the legacy docker scripts.
  - Biome の Lint 修正 (`node:` プロトコルの import、テンプレートリテラル) を適用し、`esModuleInterop` 設定と旧 docker スクリプトを削除しました。

### Container / Platform

- **Container**: Updated the Node.js engine requirement to `^22 || ^24` and the container base image to `node:24.16.0-bookworm`.
  - Node.js のエンジン要件を `^22 || ^24` に、コンテナのベースイメージを `node:24.16.0-bookworm` に更新しました。
- **Container**: Use a `localhost/` prefix to reference the locally built mirakurun image.
  - ローカルでビルドした mirakurun イメージを参照する際に `localhost/` プレフィックスを使うようにしました。

### Cleanup

- **EPGStation**: Dropped the EPGStation modules from the reboot fork and removed the leftover API references that caused a startup failure.
  - reboot フォークから EPGStation 関連モジュールを削除し、起動失敗の原因となっていた残存 API 参照を除去しました。

### Bugfix

- **Config**: Validate the `allowPNA` type before applying its default value.
  - `allowPNA` のデフォルト値を設定する前に型を検証するよう修正しました。
- **Tuners**: Emit the tuners event when tuners become available so the UI updates correctly.
  - チューナーが利用可能になった際に tuners イベントを発火し、UI が正しく更新されるよう修正しました。

## 5.0.5 (2026-06-14)

### Container / Platform

- **Container**: Migrated `docker/` to `container/` and added rootless Podman support (Quadlet `mirakurun.container` and a `podman.sh` helper with `restart`/`rebuild` subcommands).
  - `docker/` を `container/` に移行し、rootless Podman に対応しました (Quadlet `mirakurun.container` と、`restart`/`rebuild` サブコマンドを持つ `podman.sh` ヘルパーを追加)。
- **Container**: Renamed `container-init.sh` to `entrypoint.sh`, added it to the runtime stage, and switched `CMD` to an absolute path.
  - `container-init.sh` を `entrypoint.sh` にリネームし、runtime ステージへ追加して `CMD` を絶対パス化しました。
- **Container**: Access USB tuners via `/dev/bus/usb`, added a `pcscd` polkit rule, and mount the PX-M1UR `pxm1urvideo0` chardev directly for rootless setups.
  - USB チューナーへ `/dev/bus/usb` 経由でアクセスし、`pcscd` の polkit ルールを追加、rootless 構成では PX-M1UR の `pxm1urvideo0` chardev を直接マウントするようにしました。
- **Container**: Suppress `renice`/`ionice` stderr and fixed their error handling for rootless (dropped the `nice` ulimit and added the `pcscd` group to the rootless prerequisites).
  - `renice`/`ionice` の stderr を抑制し、rootless 向けにエラー処理を修正しました (`nice` の ulimit を廃止し、rootless 前提条件に `pcscd` グループを追加)。

### Config

- **Devices**: Added the PX-M1UR device and restructured the device config comments.
  - PX-M1UR デバイスを追加し、デバイス設定のコメント構成を整理しました。

### Bugfix

- **Tuners**: Strip null-valued properties from the tuners config on load.
  - チューナー設定の読み込み時に、値が null のプロパティを除去するよう修正しました。

### Project

- **Docs**: Rewrote the Platforms docs (ja/en) for the Podman-based setup, documenting the test commands and device passing; dropped `container/README.md` in favor of `doc/Platforms`.
  - Platforms ドキュメント (日本語/英語) を Podman ベースのセットアップ向けに書き直し、テストコマンドとデバイス受け渡しを記載しました。`container/README.md` は `doc/Platforms` へ統合して削除しました。

## 5.0.4 (2026-06-13)

### Config / UI

- **Guide**: Sync the Guide channel (service) visibility setting to the server via the new `/api/config/guide` endpoint (`guide.yml`), so it is shared across browsers instead of being stored in each browser only. `localStorage` is kept as an offline cache, and existing browser-only settings are migrated to the server on first connect.
  - **Guide**: 番組ガイドのチャンネル (サービス) 表示設定を、新設の `/api/config/guide` エンドポイント (`guide.yml`) 経由でサーバへ同期するようにしました。これによりブラウザ単位ではなくブラウザ間で共有されます。`localStorage` はオフラインキャッシュとして残し、既存のブラウザ単位設定は初回接続時にサーバへ自動移行します。

### Project

- **README**: Dropped Chinachu/Mirakurun upstream badges (Azure build, tip4commit, OpenCollective) and the Docker Hub section that do not apply to this fork.
  - **README**: 本フォークに該当しない Chinachu/Mirakurun 上流のバッジ (Azure build, tip4commit, OpenCollective) と Docker Hub セクションを削除しました。

## 5.0.3 (2026-06-13)

### Recording / Playback

- **Recorded**: Play raw TS in-browser via mpegts.js (MSE), with seeking over HTTP Range requests.
  - mpegts.js (MSE) によるブラウザ内 TS 再生に対応しました。シークは HTTP Range リクエストで行われます。
- **Recorded**: Transcode MPEG-2 (and other MSE-incompatible codecs) to H.264 MPEG-TS on the fly so they play in-browser; H.264/H.265 sources are still streamed as raw TS. Seeking is disabled for transcoded playback.
  - MSE で再生できない MPEG-2 等のソースを再生時に H.264 の MPEG-TS へリアルタイム変換して再生できるようにしました (H.264/H.265 は従来どおり生 TS 配信)。トランスコード再生時はシーク不可です。
- **Encode**: Track the encode source and block re-encoding when the original recording is gone.
  - エンコード元を記録し、元録画が存在しない場合の再エンコードを禁止しました。
- **Dashboard**: Added navigation from recorded items to the playback view.

### Bugfix

- **TSFilter**: Preserve external output stream listeners on close. This fixes `DELETE /api/reserves/{id}` hanging forever, which prevented recordings from being canceled or finalized.
  - TSFilter 終了時に外部出力ストリームのリスナーを保持するよう修正し、録画キャンセルがハングする問題を解消しました。
- **Recorded**: Start in-browser playback after the `<video>` element mounts. The Fluent UI dialog mounts it lazily, so playback never started and the stream was never requested.
  - 録画のブラウザ内再生を `<video>` 要素のマウント後に開始するよう修正しました。Fluent UI のダイアログが video を遅延マウントするため、再生が始まらず stream も要求されない問題を解消しました。
- **Recorded**: Use the first video stream's codec from `ffprobe` when deciding whether to transcode. Multi-program terrestrial TS emitted multiple codec lines, so MPEG-2 was streamed as raw TS that the browser could not play.
  - トランスコード要否の判定で `ffprobe` の先頭映像ストリームのコーデックを使うよう修正しました。地デジのマルチ編成 TS でコーデック名が複数行返り、MPEG-2 が再生できない生 TS のまま配信される問題を解消しました。

### UI

- Added a footer credit for the Mirakurun Reboot fork.

### Container / Tooling

- **Containerfile**: Improved build layer caching. (copy only required files, reorder build stages, add missing type definitions)

### Project

- Removed the unused azure-pipelines CI configuration.

## 5.0.2 (2026-06-13)

### UI

- **Guide**: Improved program guide. (browser-level scroll, channel mask config, date picker)
  - 番組表を改善しました。(ブラウザ標準のスクロール・チャンネルマスク設定・日付ピッカー)
- **Dashboard**: Removed a persistent horizontal scrollbar in the Recording dashboard.

### TypeScript

- Enabled `strict` mode in `src/ui` and resolved all remaining type errors.

### Container / Tooling

- **Containerfile**: Slimmed the runtime image. (dropped the build toolchain, added `curl`)
- **podman.sh**: Added a `rebuild` subcommand. (build + down + up)

### Project

- Disabled the npm-based update check and pointed links to the fork repository.

## 5.0.1 (2026-06-13)

### Key Changes

- **Project**: Renamed to `mirakurun-reboot`. (forked from Mirakurun `4.0.0-beta.16`)
- **Recording**: Merged [EPGStation](https://github.com/l3tnun/EPGStation)-style recording features into Mirakurun itself.
  - Manual reserves, rule-based auto reserves, in-process recording and recorded file management. No external process or database is required.
  - Program guide (EPG grid), program search, thumbnails, streaming playback, encode queue and storage info. (requires `ffmpeg`)
  - EPGStation のような録画予約・ルール自動予約・録画・録画済み管理を Mirakurun 本体に統合しました。外部プロセスや外部 DB は不要です。
  - 番組表 (ラテ欄)・番組検索・サムネイル・ストリーミング再生・エンコードキュー・ストレージ表示も利用できます (要 `ffmpeg`)。
  - see: [Recording](doc/Recording.ja.md)
- **TypeScript**: Enabled `strict` mode and resolved all type errors in `src`.

### Server Changes

- **Config**:
  - Added: `recording` server config section. (`recordedDirPath`, `recordedFormat`, `startMarginMs`, `endMarginMs`, `priority`, `ffmpegPath`, `thumbnailPositionSec`, `streamCommand`, `maxEncode`, `encoders`)
- **API**:
  - Added: `/reserves`, `/rules`, `/recorded`, `/schedules/search` endpoints.
  - Added: `/schedules`, `/storage`, `/encode`, `/encoders`, `/recorded/{id}/thumbnail`, `/recorded/{id}/stream` endpoints.
  - Added: `reserve` / `rule` / `recorded` / `encode` event resources.
- **UI**:
  - **RecordingView**: added. (Dashboard / Guide / Search / Reserves / Rules / Recorded / Encode)

### Bugfix

- **TSFilter**: Fixed broken service deduplication in SDT parsing.
